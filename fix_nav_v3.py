...
new_nav_block = """<ul class="nav-links" id="nav-links" role="list">
...
for filename in os.listdir(dir_path):
    ...
    # We will replace everything from <ul class="nav-links" id="nav-links" role="list"> up to <div class="nav-actions">
    # Note: <div class="nav-actions"> is right after the closing </ul>
    pattern = re.compile(r'<ul class="nav-links" id="nav-links" role="list">.*?<div class="nav-actions">', re.DOTALL)
    
    match = pattern.search(content)
    if match:
        new_nav = new_nav_block
